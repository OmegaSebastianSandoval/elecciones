...
class Administracion_informacionController extends Administracion_mainController
{
		...
		if (Session::getInstance()->get('csrf')[$this->_getSanitizedParam("csrf_section")] == $csrf) {
			...
			if ($content->info_pagina_id) {
				...
				if ($_FILES['info_logo']['name'] != '') {
					if ($content->info_logo) {
						$uploadImage->delete($content->info_logo);
					}
					$data['info_logo'] = $uploadImage->upload("info_logo");
				} else {
					$data['info_logo'] = $content->info_logo;
				}
				if ($_FILES['info_logo_grande']['name'] != '') {
					if ($content->info_logo_grande) {
						$uploadImage->delete($content->info_logo_grande);
					}
					$data['info_logo_grande'] = $uploadImage->upload("info_logo_grande");
				} else {
					$data['info_logo_grande'] = $content->info_logo_grande;
				}
	...
	private function getData()
	{
		...
		$data['info_pagina_host2'] = $this->_getSanitizedParamHtml("info_pagina_host2");
		$data['info_pagina_port2'] = $this->_getSanitizedParamHtml("info_pagina_port2");
		$data['info_pagina_username2'] = $this->_getSanitizedParamHtml("info_pagina_username2");
		$data['info_pagina_password2'] = $this->_getSanitizedParamHtml("info_pagina_password2");
		$data['info_pagina_correo_remitente2'] = $this->_getSanitizedParamHtml("info_pagina_correo_remitente2");
		$data['info_pagina_nombre_remitente2'] = $this->_getSanitizedParamHtml("info_pagina_nombre_remitente2");
		$data['info_pagina_correo_oculto2'] = $this->_getSanitizedParamHtml("info_pagina_correo_oculto2");
